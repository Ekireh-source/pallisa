...
export function MobileNavItem({ title, icon, href, subItems, isActive, onClick }: MobileNavItemProps) {
  ...
  const TriggerButton = React.forwardRef<HTMLButtonElement | HTMLAnchorElement, any>((props, ref) => {
    const className = `flex flex-col items-center justify-center flex-1 h-full min-w-[44px] min-h-[44px] gap-1 transition-colors ${
          ...
          {item.onClick ? (
            <button
              ...
              className={cn(
                ...
                item.title === "Logout" ? "text-red-600 hover:bg-red-50" : "text-gray-600 hover:bg-gray-50"
              )}
            >
              {item.icon && <Icon icon={item.icon} className={cn("w-5 h-5", item.title === "Logout" ? "text-red-500" : "text-gray-400")} />}
              {item.title}
            </button>
          ) : (
            <Link
            ...
            >
              {item.icon && <Icon icon={item.icon} className="w-5 h-5 text-gray-400" />}
              {item.title}
            </Link>
          )}
        </React.Fragment>
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="bottom" className="rounded-t-[20px] p-0 border-t-0 bg-white md:max-w-[400px] mx-auto md:mb-[100px] md:rounded-[20px] md:border md:shadow-2xl">
