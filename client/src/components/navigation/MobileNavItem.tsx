...
export function MobileNavItem({ title, icon, href, subItems, isActive }: MobileNavItemProps) {
  ...
  const TriggerButton = React.forwardRef<HTMLButtonElement | HTMLAnchorElement, any>((props, ref) => {
    ...
    if (href && (!subItems || subItems.length === 0)) {
      return (
        <Link href={href} ref={ref as any} className={className} {...props}>
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="bottom" className="rounded-t-[20px] p-0 border-t-0 bg-white">
