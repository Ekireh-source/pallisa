...
export interface SubjectSearchableSelectProps {
...
export const SubjectSearchableSelect = memo(
  ({
    value,
    onValueChange,
    disabled = false,
    ...
    const handleSelect = useCallback(
      (
        ...
        _items: PaginatedSelectItem<ISubjectListResponse>[]
      ) => {
        if (multiple) {
    ...
    return (
      <div className={`w-full !min-h-max inline-block ${className || ""}`}>
        <PaginatedSearchableSelect<ISubjectListResponse>
          paginated={true}
...
SubjectSearchableSelect.displayName = "SubjectSearchableSelect";
