  ...
  ({
    ...
    const fetchFirstPage = useCallback(
      async (query?: { search?: string; page?: number } | unknown) => {
        ...
        if (classId && classId !== "all") {
          params.class_obj = classId;
        }

        const result = await FetchStreams(params);
    ...
    const handleSelect = useCallback(
      (
        ...
        _items: PaginatedSelectItem<IStreamListResponse>[]
      ) => {
        if (multiple) {
    ...
    return (
      <div className={`w-full !min-h-max inline-block ${className || ""}`}>
        <PaginatedSearchableSelect<IStreamListResponse>
          paginated={true}
          ...
          getItemLabel={(item) => {
            const classLabel = (item as any).class_name || item.class_obj_name || "";
            return classLabel ? `${classLabel} - ${item.name}` : item.name;
          }}
...
StreamSearchableSelect.displayName = "StreamSearchableSelect";

export default StreamSearchableSelect;
