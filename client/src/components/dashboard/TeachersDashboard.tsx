...
export default function TeachersDashboard({
  ...
  const teacherCards = [
    {
      ...
      icon: UserCheck,
      href: "/students",
      colorClass: "bg-[#004aad]/10 text-[#004aad]",
    },
    {
      title: "AOI Marks",
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24 relative animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-b from-[#004aad] to-[#002f6c] text-white pt-8 pb-16 px-6 rounded-b-[40px]  relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4">
              <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 flex items-center gap-4 -xl">
      ...
      {showAdminToggle && onSwitchView && (
        <div className="max-w-4xl mx-auto px-6 -mt-6 relative z-10 flex justify-center md:justify-start">
          <div className="bg-white  ring-1 ring-gray-100/50 rounded-full p-1.5 flex gap-1">
            <button
              onClick={onSwitchView}
              className="px-5 py-1.5 rounded-full text-xs font-bold transition-all duration-200 bg-gradient-to-r from-blue-600 to-[#004aad] text-white "
            >
              Teacher Mode
            </button>
            <button
              onClick={onSwitchView}
              className="px-5 py-1.5 rounded-full text-xs font-bold transition-all duration-200 text-gray-500 hover:text-My-Black"
            >
              Admin Mode
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {teacherCards.map((card, idx) => (
            <Link href={card.href} key={idx} className="group block">
              <Card className="p-6 border border-gray-100/80 -[0_4px_12px_rgba(0,0,0,0.02)] hover:-[0_8px_24px_rgba(0,0,0,0.06)] hover:ring-2 hover:ring-blue-500/10 transition-all duration-300 rounded-[28px] bg-white flex flex-col items-center text-center hover:scale-[1.03] active:scale-[0.98] transform">
                {/* Soft Background Circle for Icon */}
                <div className={`p-4 rounded-full ${card.colorClass} mb-4 group-hover:scale-110 transition-transform duration-300 `}>
                ...
                <p className="text-[11px] text-My-Black font-semibold uppercase tracking-wider">
                  {card.description}
                </p>
              </Card>
            </Link>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button className="h-14 w-14 rounded-full bg-[#004aad] hover:bg-[#002f6c] text-white flex items-center justify-center  -blue-900/30 hover:scale-110 active:scale-95 transition-all group">
