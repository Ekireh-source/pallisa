...
interface BulkNonStaffUploadProps {
  isOpen?: boolean;
  onClose?: () => void;
  onSuccess?: () => void;
}

interface NonStaffData {
...
export function BulkNonStaffUpload({ isOpen = false, onClose, onSuccess }: BulkNonStaffUploadProps) {
  const { isAuthenticated } = useAppSelector((state) => state.auth);
  const [file, setFile] = useState<File | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState<UploadProgress | null>(null);
  const [previewData, setPreviewData] = useState<NonStaffData[]>([]);
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        headers.forEach((header, colIndex) => {
          ...
          if (value !== undefined && value !== null && value !== '') {
            switch (header) {
              case 'First Name':
                nonStaffMember.user_first_name = String(value);
                break;
              case 'Last Name':
                nonStaffMember.user_last_name = String(value);
                break;
              case 'Email':
                nonStaffMember.user_email = String(value);
                break;
              case 'Phone Number':
                nonStaffMember.user_phone = String(value);
                break;
              case 'Date of Birth (YYYY-MM-DD)':
                nonStaffMember.user_dob = String(value);
                break;
              case 'Gender (M/F/O)':
                nonStaffMember.user_gender = String(value) as 'M' | 'F' | 'O';
                break;
              case 'Emergency Contact Name':
                nonStaffMember.user_emergency_contact = String(value);
                break;
              case 'Emergency Contact Phone':
                nonStaffMember.user_emergency_phone = String(value);
                break;
              case 'Emergency Contact Address':
                nonStaffMember.user_emergency_contact_address = String(value);
                break;
              case 'Emergency Contact Email':
                nonStaffMember.user_emergency_contact_email = String(value);
                break;
              case 'Employment Type':
                nonStaffMember.employment_type = String(value) as 'full_time' | 'part_time' | 'contract' | 'substitute' | 'volunteer';
                break;
              case 'Specialization':
                nonStaffMember.specialization = String(value);
                break;
              case 'Qualification':
                nonStaffMember.qualification = String(value);
                break;
              case 'Hire Date (YYYY-MM-DD)':
                nonStaffMember.hire_date = String(value);
                break;
              case 'Years of Experience':
                nonStaffMember.years_of_experience = Number(value);
                break;
              case 'Previous Experience':
                nonStaffMember.previous_experience = String(value);
                break;
              case 'Salary (UGX)':
                nonStaffMember.salary = String(value);
                break;
            }
          }
        });
  ...
  const validateData = useCallback((data: NonStaffData[]): ValidationError[] => {
    ...
    data.forEach((nonStaffMember, index) => {
      ...
      if (!nonStaffMember.user_last_name?.trim()) {
        errors.push({ row, field: 'Last Name', message: 'Last name is required' });
      }
      if (!nonStaffMember.user_email?.trim()) {
    ...
    const template = [
      {
        ...
        'Previous Experience': 'Worked as office administrator for 3 years',
        'Salary (UGX)': '1200000'
      },
      {
        'First Name': 'Jane',
        ...
        'Previous Experience': 'Freelance IT consultant for 2 years',
        'Salary (UGX)': '800000'
      }
    ];
  ...
  const handleUpload = async () => {
    if (!isAuthenticated) {
      toast.error('Please log in to upload non-staff members.');
      return;
    }
    ...
    try {
      // Transform all non-staff members to match the API structure
      const nonStaffMembersData = previewData.map(nonStaffMember => {
        const apiData: Partial<NonStaffMemberCreateUpdate> = {
          employment_type: nonStaffMember.employment_type, // Required field
        };

        // Only add fields that have values
        if (nonStaffMember.user_email) apiData.user_email = nonStaffMember.user_email;
        if (nonStaffMember.user_first_name) apiData.user_first_name = nonStaffMember.user_first_name;
        if (nonStaffMember.user_last_name) apiData.user_last_name = nonStaffMember.user_last_name;
        if (nonStaffMember.user_gender) apiData.user_gender = nonStaffMember.user_gender;
        if (nonStaffMember.user_dob) apiData.user_dob = nonStaffMember.user_dob;
        if (nonStaffMember.user_phone) apiData.user_phone = nonStaffMember.user_phone;
        if (nonStaffMember.user_emergency_contact) apiData.user_emergency_contact = nonStaffMember.user_emergency_contact;
        if (nonStaffMember.user_emergency_phone) apiData.user_emergency_phone = nonStaffMember.user_emergency_phone;
        if (nonStaffMember.user_emergency_contact_address) apiData.user_emergency_contact_address = nonStaffMember.user_emergency_contact_address;
        if (nonStaffMember.user_emergency_contact_email) apiData.user_emergency_contact_email = nonStaffMember.user_emergency_contact_email;
        if (nonStaffMember.specialization) apiData.specialization = nonStaffMember.specialization;
        if (nonStaffMember.qualification) apiData.qualification = nonStaffMember.qualification;
        if (nonStaffMember.hire_date) apiData.hire_date = nonStaffMember.hire_date;
        if (nonStaffMember.years_of_experience !== undefined) apiData.years_of_experience = nonStaffMember.years_of_experience;
        if (nonStaffMember.previous_experience) apiData.previous_experience = nonStaffMember.previous_experience;
        if (nonStaffMember.salary) apiData.salary = nonStaffMember.salary;
  ...
  useEffect(() => {
    if (previewData.length > 0) {
      setValidationErrors(validateData(previewData));
    } else {
      setValidationErrors([]);
    }
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {!isAuthenticated && (
            <div className="border rounded-lg p-4 bg-yellow-50 border-yellow-200">
              ...
              <p className="text-sm text-yellow-800">
                Please log in to upload non-staff members. You must be authenticated to use this feature.
              </p>
            </div>
          )}
          ...
          {previewData.length > 0 && (
            <div className="space-y-2">
              <h3 className="font-medium">Preview ({previewData.length} non-staff members)</h3>
              <div className="max-h-60 overflow-y-auto border rounded-lg">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {previewData.slice(0, 10).map((nonStaffMember, index) => (
                      <tr key={index} className="border-t">
                        <td className="px-3 py-2">
                          {nonStaffMember.user_first_name} {nonStaffMember.user_last_name}
                        </td>
                        <td className="px-3 py-2">{nonStaffMember.user_email || '-'}</td>
                        <td className="px-3 py-2">{nonStaffMember.user_phone || '-'}</td>
                        <td className="px-3 py-2">{nonStaffMember.employment_type || '-'}</td>
                        <td className="px-3 py-2">{nonStaffMember.specialization || '-'}</td>
                      </tr>
                    ))}
                    {previewData.length > 10 && (
                      <tr>
                        <td colSpan={5} className="px-3 py-2 text-center text-gray-500">
                          ... and {previewData.length - 10} more non-staff members
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          >
            {isUploading ? (
              ...
              <>
                ...
                Upload {previewData.length} Non-Staff Members
              </>
            )}
          </Button>
        </div>
