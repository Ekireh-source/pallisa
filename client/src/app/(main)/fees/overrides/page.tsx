...
export default function FeeOverridesPage() {
  ...
  const totalAmount = overrides.reduce((sum, o) => sum + parseFloat(o.override_amount), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 text-white shadow-xl">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold mb-2">Student Fee Overrides</h1>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4 text-blue-100">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm">Total Overrides: {overrides.length}</span>
            </div>
            <div className="w-1 h-1 bg-blue-300 rounded-full"></div>
            <div className="flex items-center space-x-2">
              <FileText className="w-4 h-4" />
              <span className="text-sm">Active Overrides: {activeOverrides.length}</span>
            </div>
            <div className="w-1 h-1 bg-blue-300 rounded-full"></div>
            <div className="flex items-center space-x-2">
              <TrendingUp className="w-4 h-4" />
              <span className="text-sm">Total Value: {formatCurrency(totalAmount.toString())}</span>
            </div>
          </div>
          <Link
      ...
      <div className="bg-white rounded-2xl shadow-lg border-0 overflow-hidden">
        ...
        <div className="p-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by student name, category, year, or term..."
              value={searchTerm}
      ...
      {filteredOverrides.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-lg border-0 overflow-hidden">
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No overrides found</h3>
            <p className="text-gray-600 mb-6">
              {searchTerm ? 'Try adjusting your search terms.' : 'Get started by creating a new fee override.'}
            </p>
            {!searchTerm && (
              <Link
                href="/fees/overrides/create"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOverrides.map((override) => (
            <div key={override.id} className="bg-white rounded-2xl shadow-lg border-0 overflow-hidden group hover:shadow-xl transition-all duration-300">
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-600">{override.category_name}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{override.academic_year_name}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Term:</span>
                    <span className="font-medium">{override.term_name}</span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-blue-600">Total Overrides</p>
                    <p className="text-2xl font-bold text-blue-900">{overrides.length}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-900">{activeOverrides.length}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-xl p-4 border border-red-200">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-red-600">Inactive Overrides</p>
                    <p className="text-2xl font-bold text-red-900">{inactiveOverrides.length}</p>
                  </div>
                </div>
              </div>
