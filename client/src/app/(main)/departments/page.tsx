        ...
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            ...
            <p className="text-blue-100 text-lg">
              Manage organizational departments for expense tracking and reporting
            </p>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4 text-blue-100">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm">Total Departments: {departments.length}</span>
            </div>
            <div className="w-1 h-1 bg-blue-300 rounded-full"></div>
            <div className="flex items-center space-x-2">
              <FileText className="w-4 h-4" />
              <span className="text-sm">Total Expenses: {departments.reduce((sum, dept) => sum + dept.expense_count, 0)}</span>
            </div>
          </div>
          <Link
      ...
      {filteredDepartments.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredDepartments.map((department) => (
            <div key={department.id} className="bg-white rounded-2xl shadow-lg border-0 overflow-hidden group hover:shadow-xl transition-all duration-300">
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2 px-3 py-1 bg-blue-100 rounded-full">
                    ...
                    <span className="text-xs font-medium text-blue-800">
                      {department.expense_count} expenses
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-900">
                      {departments.reduce((sum, dept) => sum + dept.expense_count, 0)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-200">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-purple-600">Avg Expenses/Department</p>
                    <p className="text-2xl font-bold text-purple-900">
                      {departments.length > 0 ? Math.round(departments.reduce((sum, dept) => sum + dept.expense_count, 0) / departments.length) : 0}
                    </p>
                  </div>
                </div>
              </div>
