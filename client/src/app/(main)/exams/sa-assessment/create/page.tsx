...
import { SaAssessmentSchema, ISaAssessmentInput } from '@/features/exam/exam.schemas';
...
export default function CreateSaAssessmentPage() {
  ...
  useEffect(() => {
    const loadFormData = async () => {
      ...
      const [streamsRes, subjectsRes, teachersRes, termsRes, yearsRes] = await Promise.all([
        FetchStreams(),
        FetchSubjects(),
        FetchTeachers(),
        FetchTerms(),
  ...
  const onSubmit = async (data: any) => {
    ...
    const payload = {
      ...
      teacher: data.teacher ? parseInt(data.teacher) : null
    };

    const result = await CreateSaAssessment({ data: payload });

    if (result.success) {
      toast.success("Summative Assessment configured successfully");
      router.push('/exams/sa-assessment');
    } else {
      toast.error(result.error?.message || "Failed to configure Summative Assessment. Note: Config must be unique per Stream, Subject, Term and Year combination.");
    }
    setLoading(false);
  };
