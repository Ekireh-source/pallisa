...
export default function SubjectsListPage() {
  ...
  const loadSubjects = async () => {
    setLoading(true);
    const result = await FetchSubjects({ search: searchTerm });
    if (result.success) {
      setSubjects(result.data.results || result.data);
    } else {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage the school curriculum and course offerings.</p>
        </div>
        ...
        <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative w-full md:w-96">
            ...
            <Input 
              ...
              className="pl-10 h-10 rounded-xl border-gray-200 focus:ring-emerald-500"
              value={searchTerm}
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-gray-50/50">
              <TableRow>
                <TableHead className="w-[40%] font-semibold text-gray-900">Subject Name</TableHead>
              ...
              {loading ? (
                ...
                subjects.map((subject) => (
                  <TableRow key={subject.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <TableCell>
                      <Badge variant="outline" className="rounded-lg font-mono text-xs px-2 py-0.5 border-gray-200">
                        {subject.code}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <span className="text-sm text-gray-600">{subject.category || 'Core'}</span>
                    </TableCell>
                    <TableCell>
                      <Badge 
