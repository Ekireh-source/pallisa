...
export default function StudentDetailPage({ params }: PageProps) {
  ...
  const DetailItem = ({ icon: Icon, label, value }: any) => (
    <div className="flex items-start gap-4 p-4 rounded-2xl hover:bg-gray-50/80 transition-all border border-transparent hover:border-gray-100">
      <div className="p-2.5 rounded-xl bg-primary/5 text-primary border border-primary/10 flex items-center justify-center shrink-0">
        <Icon className="w-4 h-4" />
      </div>
      <div className="space-y-1">
        <p className="text-xs font-semibold text-My-Black uppercase tracking-wider">{label}</p>
        <p className="text-sm font-bold text-My-Black">{value || 'N/A'}</p>
      </div>
    </div>
  );
  ...
  return (
    <ProtectedComponent permissionCode={PERMISSION_CODES.VIEW_STUDENTS}>
      <MainLayout
        ...
        description={`ID: ${student.student_id} • ${student.current_class_name || ''} • ${student.current_stream_name || ''}`}
        backButton={
          <Button
            variant="ghost"
            size="icon"
      ...
      >
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 mt-6">
          ...
          <Card className="p-8 border-none rounded-[32px] ring-1 ring-gray-100 overflow-hidden relative group bg-white">
            <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full -mr-24 -mt-24 transition-transform group-hover:scale-110 duration-700 pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
              ...
              <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left w-full lg:w-auto">
                <div className="w-28 h-28 rounded-[2rem] bg-primary/10 p-1 ring-4 ring-primary/5 flex items-center justify-center overflow-hidden shrink-0">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                    <h2 className="text-3xl font-extrabold text-My-Black tracking-tight">{student.full_name}</h2>
                    <Badge className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-widest border-none ${student.enrollment_status === 'enrolled'
                  ...
                  <p className="text-gray-500 font-medium text-sm flex flex-wrap items-center justify-center md:justify-start gap-x-2 gap-y-1">
                    <span className="text-primary font-bold">ID: {student.student_id}</span>
                    {student.lin && (
                      <>
                    ...
                    <span className="w-1.5 h-1.5 rounded-full bg-gray-300 hidden md:inline" />
                    <span className="text-My-Black font-semibold bg-gray-100 px-2.5 py-0.5 rounded-full text-xs">{student.current_class_name} • {student.current_stream_name}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full lg:w-auto shrink-0">
                <div className="p-4 rounded-2xl bg-gray-50/80 border border-gray-100 text-center min-w-[110px]">
                  <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest mb-1">Gender</p>
                  <p className="text-base font-extrabold text-My-Black">{student.user_profile_data?.gender === 'M' ? 'Male' : 'Female'}</p>
                </div>
                <div className="p-4 rounded-2xl bg-gray-50/80 border border-gray-100 text-center min-w-[110px]">
                  <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest mb-1">Age</p>
                  <p className="text-base font-extrabold text-My-Black">{student.age || 'N/A'}</p>
                </div>
                <div className="p-4 rounded-2xl bg-gray-50/80 border border-gray-100 text-center min-w-[110px]">
                  <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest mb-1">Stream</p>
                  <p className="text-base font-extrabold text-My-Black truncate max-w-[100px]">{student.current_stream_name || 'N/A'}</p>
                </div>
                <div className="p-4 rounded-2xl bg-gray-50/80 border border-gray-100 text-center min-w-[110px]">
                  <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest mb-1">Admission No</p>
                  <p className="text-base font-extrabold text-My-Black truncate max-w-[100px]">{student.admission_number || 'N/A'}</p>
                </div>
              </div>
            </div>
          </Card>
          ...
          <div className="w-full space-y-6">
            <Card className="p-2 border-none rounded-[32px] ring-1 ring-gray-100 bg-white">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-transparent border-b border-gray-100 w-full justify-start h-auto p-4 gap-2 overflow-x-auto flex-wrap">
                  <TabsTrigger
                    value="marks"
                    className="rounded-xl px-6 py-3 font-bold text-sm data-[state=active]:bg-primary data-[state=active]:text-white transition-all flex items-center gap-2"
                  >
                    ...
                    {reportCards && reportCards.length > 0 ? (
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-primary/5 border border-primary/20">
                          <div>
                            <h4 className="text-lg font-bold text-My-Black">Term Progress & Subject Scores</h4>
                            <p className="text-xs font-semibold text-gray-500 mt-1">Select an active term report card to view detailed results.</p>
                          </div>

                          <div className="flex items-center gap-3">
                            <Label className="text-sm font-bold text-My-Black shrink-0 hidden sm:inline">Active Report:</Label>
                            <Select value={selectedReportCardId} onValueChange={setSelectedReportCardId}>
                              <SelectTrigger className="w-[260px] h-11 rounded-xl bg-white border-gray-200 font-bold text-My-Black">
                        ...
                        {activeReportCard && (
                          <div className="space-y-6">
                                      ...
                                      <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                        <td className="px-6 py-4">
                                          <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                              <p className="text-sm font-bold text-My-Black">{sr.subject_name}</p>
                                              <p className="text-[10px] text-My-Black font-semibold">{sr.subject_code || '—'}</p>
                                            </div>
                                          </div>
                                        </td>
                                        <td className="px-6 py-4 text-center text-sm font-bold text-My-Black">
                                        ...
                                        <td className="px-6 py-4 text-sm font-semibold text-My-Black">
                                          {sr.remarks || '—'}
                                        </td>
                                      </tr>
                                    ))
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                              <Card className="p-5 border-none bg-primary/5 border border-primary/20 flex items-center gap-4 rounded-2xl">
                                <div className="p-3 bg-white rounded-xl text-primary">
                                  <Icon icon="hugeicons:analytics-up" className="w-6 h-6" />
                                </div>
                                <div>
                                  <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest">Overall Average</p>
                              ...
                              <Card className="p-5 border-none bg-emerald-50/50 border border-emerald-100 flex items-center gap-4 rounded-2xl">
                                <div className="p-3 bg-white rounded-[12px] text-emerald-600">
                                  <Icon icon="hugeicons:award-02" className="w-6 h-6" />
                                </div>
                                <div>
                                  <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest font-black">Grade / Aggregate</p>
                              ...
                              <Button
                                variant="outline"
                                className="h-full rounded-2xl border-primary/20 hover:bg-primary/10 text-primary font-bold p-5 flex items-center justify-center gap-3 transition-all cursor-pointer"
                      ...
                      <div className="flex flex-col items-center justify-center py-16 text-center w-full">
                        <div className="w-20 h-20 rounded-[2rem] bg-primary/10 flex items-center justify-center text-primary mb-6 ring-8 ring-primary/5">
                          <Icon icon="hugeicons:graduation-cap" className="w-10 h-10" />
                        </div>
                        <h4 className="text-xl font-bold text-My-Black mb-2">No Results Recorded Yet</h4>
                        ...
                        <Button
                          onClick={() => setShowGenModal(true)}
                          className="rounded-xl px-6 h-11 bg-primary text-white hover:bg-primary/90 font-bold flex items-center gap-2"
                        >
                  ...
                  <TabsContent value="academic" className="m-0 space-y-8 animate-in fade-in duration-500 focus-visible:outline-none">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-My-Black uppercase tracking-widest mb-2">Enrollment Information</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <DetailItem icon={GraduationCap} label="Current Stream" value={`${student.current_class_name} - ${student.current_stream_name}`} />
                        </div>
                      </div>

                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-My-Black uppercase tracking-widest mb-2">Contact Info & Previous Education</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="pt-8 border-t border-gray-100">
                      <h3 className="text-base font-extrabold text-My-Black mb-6 flex items-center gap-3">
                        <ShieldAlert className="w-5 h-5 text-primary" />
                        Emergency Contact Details
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="p-5 rounded-2xl bg-primary/5 border border-primary/20">
                          <p className="text-sm font-bold text-primary mb-1">{student.user_profile_data?.emergency_contact || 'None Listed'}</p>
                          <p className="text-[10px] font-bold text-primary/70 uppercase tracking-widest">Primary Contact Name</p>
                        </div>
                        <div className="p-5 rounded-2xl bg-gray-50 border border-gray-100">
                          <p className="text-sm font-bold text-My-Black mb-1">{student.user_profile_data?.emergency_phone || 'N/A'}</p>
                          <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest">Emergency Phone</p>
                        </div>
                        <div className="p-5 rounded-2xl bg-gray-50 border border-gray-100">
                          <p className="text-sm font-bold text-My-Black mb-1">{student.user_profile_data?.emergency_contact_email || 'N/A'}</p>
                          <p className="text-[10px] font-bold text-My-Black uppercase tracking-widest">Emergency Email</p>
                        </div>
                      </div>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="medical" className="m-0 space-y-8 animate-in fade-in duration-500 focus-visible:outline-none">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="p-6 rounded-2xl bg-rose-50 border border-rose-100">
                        ...
                        <p className="text-sm font-bold text-My-Black">{student.medical_conditions || 'None Listed'}</p>
                      </div>
                      <div className="p-6 rounded-2xl bg-amber-50 border border-amber-100">
                        ...
                        <p className="text-sm font-bold text-My-Black">{student.allergies || 'None Listed'}</p>
                      </div>
                      <div className="p-6 rounded-2xl bg-blue-50 border border-blue-100">
                        ...
                        <p className="text-sm font-bold text-My-Black">{student.special_needs || 'None Listed'}</p>
                      </div>
                    </div>
                  </TabsContent>
                      ...
                      <div className="space-y-8">
                        <div className="relative pl-14">
                          ...
                          <div className="p-6 rounded-2xl bg-gray-50 border border-gray-100">
                            ...
                            <h5 className="font-bold text-My-Black mb-1">{student.current_class_name} • {student.current_stream_name}</h5>
                        ...
                        <div className="relative pl-14">
                          ...
                          <div className="p-6 rounded-2xl bg-white ring-1 ring-gray-100">
                            <p className="text-xs font-bold text-My-Black uppercase tracking-widest mb-1">Account Created</p>
        ...
        <Dialog open={showGenModal} onOpenChange={setShowGenModal}>
          <DialogContent className="rounded-[32px] p-8 border-none max-w-md">
            <DialogHeader>
              <DialogTitle className="text-2xl font-extrabold text-My-Black mb-2">Generate Report Card</DialogTitle>
              <p className="text-gray-500 font-medium">Select the academic period for {student.full_name}'s report card.</p>
            </DialogHeader>

            <div className="space-y-6 py-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-My-Black uppercase tracking-widest px-1">Academic Year</label>
                <AcademicYearSearchableSelect
                  value={selectedYear}
                  onValueChange={setSelectedYear}
                  placeholder="Select Year"
                  triggerClassName="h-12 rounded-2xl border-gray-100 bg-gray-50/50 focus:ring-primary font-bold text-My-Black"
                />
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-My-Black uppercase tracking-widest px-1">Term</label>
            ...
            <DialogFooter className="gap-3 sm:justify-between">
              ...
              <Button
                ...
                disabled={generating}
                className="rounded-2xl h-12 px-8 font-bold bg-primary hover:bg-primary/90 transition-all min-w-[160px]"
              >
                {generating ? (
                  <>
