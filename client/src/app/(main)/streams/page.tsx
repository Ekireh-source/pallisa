...
export default function StreamsListPage() {
  ...
  const loadStreams = async () => {
    setLoading(true);
    const result = await FetchStreams({ search: searchTerm });
    if (result.success) {
      setStreams(result.data.results || result.data);
    } else {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage class divisions and student distributions.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-gray-50/50">
              <TableRow>
                <TableHead className="w-[30%] font-semibold text-gray-900">Stream Name</TableHead>
                ...
                <TableHead className="font-semibold text-gray-900">Enrollment</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                streams.map((stream) => (
                  <TableRow key={stream.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        <span className="text-sm text-gray-900 font-medium">{stream.class_obj_name || 'N/A'}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <span className="text-sm text-gray-600">{stream.capacity} students</span>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Users className="w-3.5 h-3.5 text-gray-400" />
                        <span className="text-sm text-gray-600">
                          {stream.current_enrollment || 0} / {stream.capacity}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
