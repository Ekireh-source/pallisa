...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <Card className="w-full max-w-md p-8 border-none shadow-2xl shadow-red-100 rounded-[32px] text-center space-y-6">
  ...
  if (submitted) {
    return (
      <Card className="w-full max-w-md p-8 border-none shadow-2xl shadow-indigo-100 rounded-[32px] text-center space-y-6 animate-in zoom-in-95 duration-500">
        <div className="w-20 h-20 bg-emerald-50 rounded-3xl flex items-center justify-center mx-auto mb-6">
          <CheckCircle2 className="w-10 h-10 text-emerald-500" />
        </div>
        <h1 className="text-3xl font-extrabold text-gray-900">Password Reset!</h1>
        <p className="text-gray-500 leading-relaxed">
  ...
  return (
    <Card className="w-full max-w-md p-8 border-none shadow-2xl shadow-indigo-100 rounded-[32px] space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
          <Label htmlFor="new_password" className="text-sm font-bold text-gray-700 ml-1">New Password</Label>
        ...
        <div className="space-y-2">
          <Label htmlFor="confirm_password" className="text-sm font-bold text-gray-700 ml-1">Confirm New Password</Label>
          <div className="relative group">
            <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-indigo-500 transition-colors" />
            <Input 
              id="confirm_password"
        ...
        <Button 
          type="submit" 
          className="w-full h-14 rounded-2xl shadow-xl shadow-indigo-200 font-extrabold text-lg transition-all hover:scale-[1.02] active:scale-95 bg-indigo-600 hover:bg-indigo-700"
          disabled={loading}
        >
          {loading ? (
