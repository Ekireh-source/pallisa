...
<head>
    ...
    <title>Subjects Management | Admin Workspace</title>
...
<body class="text-slate-800 antialiased h-screen flex relative">
    ...
    <aside class="w-64 bg-[#052c54] flex flex-col h-full flex-shrink-0 border-r border-[#041e3a] relative z-20 shadow-xl overflow-hidden">
        ...
        <nav class="flex-1 py-4 flex flex-col gap-0.5 px-3 relative z-10 overflow-hidden">
            ...
            <button onclick="navigateTo('admin_subjects.php')" class="sidebar-link active flex items-center gap-2.5 px-3 py-2 rounded-lg border-l-4 border-transparent text-xs font-semibold w-full text-left">
    ...
    <main class="flex-1 flex flex-col h-full relative bg-slate-50/50">
        <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 z-10 flex-shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center border border-purple-100"><i data-lucide="book-open" class="w-4 h-4"></i></div>
                <h2 class="text-lg font-extrabold text-slate-800 tracking-tight">Subject Registry</h2>
            </div>
        </header>
            ...
            <div class="grid grid-cols-12 gap-5 flex-1 min-h-0">
                ...
                <div class="col-span-4 flex flex-col gap-5 overflow-y-auto">
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 elevate-card">
                        <div class="flex items-center gap-2.5 mb-4 pb-3 border-b border-slate-100">
                            ...
                            <h3 class="font-extrabold text-slate-800 text-sm">Register New Subject</h3>
                        </div>
                        <form id="formAddSubject" class="space-y-4">
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Subject Name</label>
                                <input type="text" id="inpSubName" placeholder="e.g., Physics" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all">
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Subject Code</label>
                                <input type="text" id="inpSubCode" placeholder="e.g., PHY" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all uppercase">
                            </div>
                            <button type="submit" id="btnSaveSubject" class="w-full bg-brand hover:bg-brand-dark text-white py-2 rounded-lg text-xs font-bold shadow-sm active:scale-95 transition-all flex justify-center items-center gap-2">
                                <i data-lucide="save" class="w-3.5 h-3.5"></i> Save Subject
                            </button>
                        </form>
                    </div>

                    <div class="bg-gradient-to-br from-indigo-50 to-blue-50 p-5 rounded-xl border border-blue-100">
                        <h4 class="text-xs font-extrabold text-brand flex items-center gap-2 mb-2"><i data-lucide="info" class="w-4 h-4"></i> Registry Guidelines</h4>
                        <p class="text-[11px] text-slate-600 leading-relaxed">Subjects are the core pillars of the curriculum. Ensure that subject codes are completely unique (e.g., MTC, ENG). These codes will appear on all exported Excel reports.</p>
                    </div>
                </div>
                ...
                <div class="col-span-8 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
                        <div>
                            <h3 class="text-base font-extrabold text-slate-800">Master Subjects List</h3>
                            <p class="text-[11px] text-slate-500 mt-0.5">Manage and edit the school's curriculum offerings.</p>
                        </div>
                        <div class="relative">
                            <i data-lucide="search" class="w-3.5 h-3.5 absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400"></i>
                            <input type="text" id="inpSearchSub" onkeyup="filterSubjects()" placeholder="Search subjects..." class="pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium focus:outline-none focus:border-brand w-48">
                        </div>
                    </div>

                    <div id="subjectsContainer" class="flex-1 overflow-y-auto p-5 grid grid-cols-2 gap-4 bg-slate-50/30 content-start">
                        <div class="col-span-2 text-center py-10 text-slate-400"><i data-lucide="loader-2" class="w-5 h-5 animate-spin mx-auto mb-2 text-brand"></i> Compiling curriculum...</div>
                    </div>
                </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-95 transition-transform duration-300" id="editModalContent">
            ...
            <h3 class="text-lg font-extrabold text-slate-800 mb-1">Edit Subject</h3>
            <p class="text-xs text-slate-500 mb-5 leading-relaxed">Changes will instantly apply to all teacher workloads.</p>
            
            <div class="space-y-4 mb-6">
                <div class="space-y-1.5">
                    <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Subject Name</label>
                    <input type="text" id="inpEditName" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2.5 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 transition-all">
                </div>
                <div class="space-y-1.5">
                    <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Subject Code</label>
                    <input type="text" id="inpEditCode" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2.5 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 transition-all uppercase">
                </div>
            </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-95 transition-transform duration-300" id="deleteModalContent">
            ...
            <h3 class="text-lg font-extrabold text-slate-800 mb-1">Delete Subject?</h3>
            <p class="text-xs text-slate-500 mb-5 leading-relaxed">You are about to delete <strong id="deleteTargetName" class="text-slate-800"></strong>. <span class="text-rose-500 font-bold">This will orphan related scores and teacher assignments!</span></p>
    ...
    <script src="admin_subjects.js"></script>
</body>
</html>