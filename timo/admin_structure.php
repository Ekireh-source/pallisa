...
<head>
    ...
    <title>Classes & Streams | Admin Workspace</title>
...
<body class="text-slate-800 antialiased h-screen flex relative">
    ...
    <aside class="w-64 bg-[#052c54] flex flex-col h-full flex-shrink-0 border-r border-[#041e3a] relative z-20 shadow-xl overflow-hidden">
        ...
        <nav class="flex-1 py-4 flex flex-col gap-0.5 px-3 relative z-10 overflow-hidden">
            ...
            <button onclick="navigateTo('admin_structure.php')" class="sidebar-link active flex items-center gap-2.5 px-3 py-2 rounded-lg border-l-4 border-transparent text-xs font-semibold w-full text-left">
    ...
    <main class="flex-1 flex flex-col h-full relative bg-slate-50/50">
        <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 z-10 flex-shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-blue-50 text-brand flex items-center justify-center border border-blue-100"><i data-lucide="building" class="w-4 h-4"></i></div>
                <h2 class="text-lg font-extrabold text-slate-800 tracking-tight">Classes & Streams Structure</h2>
            </div>
        </header>
            ...
            <div class="grid grid-cols-12 gap-5 flex-1 min-h-0">
                ...
                <div class="col-span-4 flex flex-col gap-5 overflow-y-auto">
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 elevate-card">
                        <div class="flex items-center gap-2.5 mb-4 pb-3 border-b border-slate-100">
                            <div class="w-7 h-7 rounded-lg bg-blue-50 text-brand flex items-center justify-center"><i data-lucide="plus-circle" class="w-4 h-4"></i></div>
                            <h3 class="font-extrabold text-slate-800 text-sm">Create New Class</h3>
                        </div>
                        <form id="formAddClass" class="space-y-4">
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Class Name</label>
                                <input type="text" id="inpClassName" placeholder="e.g., Senior 5" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all">
                            </div>
                            <button type="submit" id="btnSaveClass" class="w-full bg-brand hover:bg-brand-dark text-white py-2 rounded-lg text-xs font-bold shadow-sm active:scale-95 transition-all flex justify-center items-center gap-2">
                                <i data-lucide="save" class="w-3.5 h-3.5"></i> Save Class
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 elevate-card">
                        <div class="flex items-center gap-2.5 mb-4 pb-3 border-b border-slate-100">
                            <div class="w-7 h-7 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center"><i data-lucide="layers" class="w-4 h-4"></i></div>
                            <h3 class="font-extrabold text-slate-800 text-sm">Attach a Stream</h3>
                        </div>
                        <form id="formAddStream" class="space-y-4">
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Parent Class</label>
                                <select id="selParentClass" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all">
                                    <option value="">-- Select --</option>
                                </select>
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Stream Name</label>
                                <input type="text" id="inpStreamName" placeholder="e.g., S5 Science" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all">
                            </div>
                            <button type="submit" id="btnSaveStream" class="w-full bg-amber-500 hover:bg-amber-600 text-white py-2 rounded-lg text-xs font-bold shadow-sm active:scale-95 transition-all flex justify-center items-center gap-2">
                                <i data-lucide="save" class="w-3.5 h-3.5"></i> Attach Stream
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-span-8 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
                        <div>
                            <h3 class="text-base font-extrabold text-slate-800">Hierarchy Directory</h3>
                            <p class="text-[11px] text-slate-500 mt-0.5">Manage, edit, or delete classes and streams.</p>
                        </div>
                        <button onclick="loadStructureData()" class="w-8 h-8 rounded-md bg-white border border-slate-200 text-slate-500 hover:text-brand hover:border-brand hover:bg-blue-50 transition-colors flex items-center justify-center shadow-sm">
                            <i data-lucide="refresh-cw" class="w-4 h-4"></i>
                        </button>
                    </div>

                    <div id="structureContainer" class="flex-1 overflow-y-auto p-5 space-y-3 bg-slate-50/30">
                        <div class="text-center py-10 text-slate-400"><i data-lucide="loader-2" class="w-5 h-5 animate-spin mx-auto mb-2 text-brand"></i> Building hierarchy...</div>
                    </div>
                </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-95 transition-transform duration-300" id="editModalContent">
            ...
            <h3 class="text-lg font-extrabold text-slate-800 mb-1" id="editModalTitle">Rename</h3>
            <p class="text-xs text-slate-500 mb-5 leading-relaxed">Update the name across the entire school system.</p>
            
            <div class="space-y-2 mb-6">
                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest" id="editModalLabel">New Name</label>
                <input type="text" id="inpEditName" autocomplete="off" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2.5 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 transition-all">
            </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-95 transition-transform duration-300" id="deleteModalContent">
            ...
            <h3 class="text-lg font-extrabold text-slate-800 mb-1" id="deleteModalTitle">Delete Data?</h3>
            <p class="text-xs text-slate-500 mb-5 leading-relaxed" id="deleteModalWarning">You are about to delete <strong id="deleteTargetName" class="text-slate-800"></strong>. <span class="text-rose-500 font-bold">This cannot be undone.</span></p>
            
            <div class="space-y-2 mb-6">
                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest flex items-center justify-between">
                    <span>Type "delete" to confirm</span>
                </label>
                <input type="text" id="inpDeleteConfirm" autocomplete="off" placeholder="delete" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2.5 focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all text-center">
            </div>
    ...
    <script src="admin_structure.js"></script>
</body>
</html>