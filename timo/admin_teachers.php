...
<body class="text-slate-800 antialiased h-screen flex relative">
    ...
    <aside class="w-64 bg-[#052c54] flex flex-col h-full flex-shrink-0 border-r border-[#041e3a] relative z-20 shadow-xl overflow-hidden">
        ...
        <nav class="flex-1 py-4 flex flex-col gap-0.5 px-3 relative z-10 overflow-hidden">
            ...
            <button onclick="navigateTo('admin_teachers.php')" class="sidebar-link active flex items-center gap-2.5 px-3 py-2 rounded-lg border-l-4 border-transparent text-xs font-semibold w-full text-left">
    ...
    <main class="flex-1 flex flex-col h-full relative bg-slate-50/50">
        <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 z-10 flex-shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100"><i data-lucide="users" class="w-4 h-4"></i></div>
            ...
            <div class="grid grid-cols-12 gap-5 flex-1 min-h-0">
                    ...
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 elevate-card">
                        ...
                        <form id="formAddTeacher" class="space-y-4">
                            <div class="space-y-1.5">
                                ...
                                <input type="text" id="inpTeacherName" placeholder="e.g., Joram Bwambale" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all">
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Email Address</label>
                                <input type="email" id="inpTeacherEmail" placeholder="teacher@school.com" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2 focus:outline-none focus:border-brand transition-all">
                            </div>
                            <div class="bg-blue-50/50 border border-blue-100 p-3 rounded-lg flex items-start gap-2">
                                ...
                                <p class="text-[10px] text-slate-600 font-medium">New accounts are automatically assigned the default password: <strong class="text-slate-800 font-mono tracking-wider">teacher123</strong></p>
                            </div>
                            <button type="submit" id="btnSaveTeacher" class="w-full bg-brand hover:bg-brand-dark text-white py-2 rounded-lg text-xs font-bold shadow-sm active:scale-95 transition-all flex justify-center items-center gap-2 mt-2">
                ...
                <div class="col-span-8 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
                        <div>
                            <h3 class="text-base font-extrabold text-slate-800 flex items-center gap-2">Staff Roster <span id="staffCountBadge" class="bg-slate-200 text-slate-600 text-[10px] px-2 py-0.5 rounded-full">0</span></h3>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="inpSearchTeacher" onkeyup="filterTeachers()" placeholder="Search name or email..." class="pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium focus:outline-none focus:border-brand w-56 shadow-sm">
                        </div>
                    </div>

                    <div id="teachersContainer" class="flex-1 overflow-y-auto p-5 space-y-3 bg-slate-50/30 content-start">
                        <div class="text-center py-10 text-slate-400"><i data-lucide="loader-2" class="w-5 h-5 animate-spin mx-auto mb-2 text-brand"></i> Compiling roster...</div>
                    </div>
                </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-95 transition-transform duration-300" id="editModalContent">
            ...
            <div class="space-y-4 mb-6">
                ...
                <div class="space-y-1.5">
                    <label class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Email Address</label>
                    <input type="email" id="inpEditEmail" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-lg px-3 py-2.5 focus:outline-none focus:border-brand transition-all">
                </div>
            </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-95 transition-transform duration-300" id="deleteModalContent">
            ...
            <p class="text-xs text-slate-500 mb-5 leading-relaxed">You are about to delete <strong id="deleteTargetName" class="text-slate-800"></strong>. <span class="text-rose-500 font-bold">If this teacher has logged attendance records, this action will be blocked to protect school history.</span></p>
