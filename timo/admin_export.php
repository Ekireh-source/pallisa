...
<head>
    ...
    <style>
        ...
        .elevate-card { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
    </style>
</head>
<body class="text-slate-800 antialiased h-screen flex relative">
    ...
    <aside class="w-64 bg-[#052c54] flex flex-col h-full flex-shrink-0 border-r border-[#041e3a] relative z-20 shadow-xl overflow-hidden">
        ...
        <nav class="flex-1 py-4 flex flex-col gap-0.5 px-3 relative z-10 overflow-hidden">
            ...
            <button onclick="navigateTo('admin_export.php')" class="sidebar-link active flex items-center gap-2.5 px-3 py-2 rounded-lg border-l-4 border-transparent text-xs font-semibold w-full text-left text-cyan-300">
    ...
    <main class="flex-1 flex flex-col h-full relative bg-slate-50/50">
        ...
        <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 z-10 flex-shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100"><i data-lucide="download-cloud" class="w-4 h-4"></i></div>
        ...
        <div class="flex-1 overflow-y-auto p-8 flex flex-col items-center gap-8">
            ...
            <div class="w-full max-w-4xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-shrink-0">
                
                <div class="w-2/5 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-slate-900 via-brand-dark to-brand p-10 text-white relative overflow-hidden flex flex-col justify-between">
                    <div class="absolute top-0 right-0 w-48 h-48 bg-white/10 rounded-full blur-3xl -translate-y-10 translate-x-10"></div>
                    <i data-lucide="file-spreadsheet" class="w-40 h-40 absolute -right-10 -bottom-10 opacity-10"></i>
                    
                    <div class="relative z-10">
                        <div class="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center mb-6 border border-white/20 backdrop-blur-md">
                            <i data-lucide="table" class="w-6 h-6 text-white"></i>
                        </div>
                        <h2 class="text-3xl font-extrabold tracking-tight mb-3">Formula Engine</h2>
                        <p class="text-blue-100 text-sm leading-relaxed mb-6 font-medium opacity-90">Preview or download perfectly formatted academic records. The engine automatically applies the C1 rule mapping and performs massive grid aggregations.</p>
                ...
                <div class="w-3/5 p-8 flex flex-col">
                    <form id="formExport" class="space-y-6 flex-1 flex flex-col justify-center">
                        ...
                        <div class="space-y-2">
                            <label class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest flex items-center gap-2"><i data-lucide="file-bar-chart" class="w-4 h-4"></i> Select Report Type</label>
                            <select id="selReportType" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-xl px-4 py-3.5 focus:outline-none focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all cursor-pointer">
                        ...
                        <div class="grid grid-cols-2 gap-5 transition-all duration-300">
                            <div class="space-y-2 w-full col-span-1" id="wrapStream">
                                <label class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Target Stream</label>
                                <select id="selStream" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-xl px-4 py-3.5 focus:outline-none focus:border-brand transition-all cursor-pointer">
                                    <option value="">-- Select Stream --</option>
                                </select>
                            </div>

                            <div class="space-y-2 w-full col-span-1 transition-all duration-300" id="wrapSubject">
                                <label class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Subject Filter</label>
                                <select id="selSubject" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold rounded-xl px-4 py-3.5 focus:outline-none focus:border-brand transition-all cursor-pointer">
                        ...
                        <div class="grid grid-cols-2 gap-3 pt-6 border-t border-slate-100">
                            <button type="button" onclick="previewData()" class="w-full bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-3.5 rounded-xl text-sm font-extrabold shadow-sm active:scale-95 transition-all flex justify-center items-center gap-2 border border-indigo-200">
                                <i data-lucide="eye" class="w-4 h-4"></i> Preview Data
                            </button>
                            <button type="button" onclick="downloadData()" class="w-full bg-slate-800 hover:bg-slate-900 text-white py-3.5 rounded-xl text-sm font-extrabold shadow-lg shadow-slate-900/20 active:scale-95 transition-all flex justify-center items-center gap-2">
            ...
            <div id="previewPane" class="w-full max-w-6xl bg-white rounded-2xl shadow-sm border border-slate-200 flex-col elevate-card hidden flex-shrink-0 mb-8">
                <div class="p-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center rounded-t-2xl">
                    <h3 class="font-extrabold text-slate-800 text-sm flex items-center gap-2"><i data-lucide="table" class="w-4 h-4 text-brand"></i> <span id="lblPreviewTitle">Live Data Preview</span></h3>
                    <button onclick="closePreview()" class="text-slate-400 hover:text-rose-500 transition-colors"><i data-lucide="x-circle" class="w-5 h-5"></i></button>
                </div>
